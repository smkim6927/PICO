# eval/modules/n_cl_evaluator.py
...
def _require_eval_runner_revision() -> None:
    parameters = inspect.signature(EvalRunner.__init__).parameters
    required = {"meteor_mode", "metric_tokenizer", "compute_token_metrics", "compatibility_tokenizers"}
    missing = sorted(required - set(parameters))
    if missing:
        raise RuntimeError(
            "The installed modules.n_eval_stability.EvalRunner is too old for "
            f"multilingual CL evaluation; missing constructor arguments: {missing}. "
            "Use the companion revised n_eval_stability.py."
        )

    # Fail before loading checkpoints if the companion EvalRunner does not expose
    # the chrF-aware failed-domain schema.
    import modules.n_eval_stability as eval_stability_module

    nan_factory = getattr(eval_stability_module, "_nan_metrics", None)
    if not callable(nan_factory) or "chrf" not in nan_factory():
        raise RuntimeError(
            "modules.n_eval_stability is not the chrF-aware revision. Replace it "
            "with the companion n_eval_stability.py before running CL evaluation."
        )
...
def _tokenizer_fingerprint(tokenizer: Any) -> Dict[str, Any]:
    try:
        ...
        raise RuntimeError("Cannot fingerprint tokenizer vocabulary.") from exc

    vocab_hasher = hashlib.sha256()
    for token, token_id in sorted(vocab.items(), key=lambda item: (int(item[1]), item[0])):
        vocab_hasher.update(str(int(token_id)).encode("ascii"))
        vocab_hasher.update(b"\0")
        vocab_hasher.update(str(token).encode("utf-8", errors="surrogatepass"))
        vocab_hasher.update(b"\0")
    ...
    if backend_tokenizer is not None and hasattr(backend_tokenizer, "to_str"):
        try:
            backend_hash = _hash_text(backend_tokenizer.to_str())
        except Exception:
            backend_hash = None
    ...
    compact = {
        ...
        "vocab_hash": vocab_hasher.hexdigest(),
        "backend_hash": backend_hash,
        "special_tokens_map": _stable_json_value(
            getattr(tokenizer, "special_tokens_map", {})
        ),
        "special_token_ids": {
            "pad": _stable_json_value(getattr(tokenizer, "pad_token_id", None)),
            "eos": _stable_json_value(getattr(tokenizer, "eos_token_id", None)),
            "bos": _stable_json_value(getattr(tokenizer, "bos_token_id", None)),
            "unk": _stable_json_value(getattr(tokenizer, "unk_token_id", None)),
        },
        "chat_template_hash": _hash_text(str(getattr(tokenizer, "chat_template", ""))),
    }
    canonical = json.dumps(compact, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
    return {
        **compact,
        "fingerprint": _hash_text(canonical),
    }


def _tokenizer_metadata_diff(
    reference: Mapping[str, Any], current: Mapping[str, Any]
) -> Dict[str, Dict[str, Any]]:
    """Return compact, auditable tokenizer changes between checkpoints."""
    keys = (
        "class",
        "length",
        "vocab_hash",
    ...
    for key in keys:
        before = _stable_json_value(reference.get(key))
        after = _stable_json_value(current.get(key))
        if before != after:
            diff[key] = {"base": before, "checkpoint": after}
    return diff


def _model_fingerprint(model: Any) -> Dict[str, Any]:
    """Fingerprint stable backbone architecture separately from vocab interface.

    Continual pre-training may legitimately update the tokenizer and resize the
    input/output embeddings.  Those fields therefore must not be folded into the
    architecture identity check.  Hidden size, depth, attention layout, RoPE, and
    decoder/encoder family still have to remain unchanged.
    """
    config = getattr(model, "config", None)
    if config is None:
        raise RuntimeError("Loaded model does not expose a config object.")

    input_embeddings = model.get_input_embeddings()
    if input_embeddings is None or not hasattr(input_embeddings, "weight"):
        raise RuntimeError("Model does not expose input embeddings for fingerprinting.")
    ...
    architecture = {
        field: _stable_json_value(getattr(config, field, None))
        for field in architecture_fields
    }
    architecture_json = json.dumps(
        architecture, sort_keys=True, ensure_ascii=False, separators=(",", ":")
    )

    vocab_interface = {
        "config_vocab_size": _stable_json_value(getattr(config, "vocab_size", None)),
        "input_embedding_rows": int(input_embeddings.weight.shape[0]),
        "output_embedding_rows": output_rows,
    }
    interface_json = json.dumps(
        vocab_interface, sort_keys=True, ensure_ascii=False, separators=(",", ":")
    )

    full = {"architecture": architecture, "vocab_interface": vocab_interface}
    full_json = json.dumps(
        full, sort_keys=True, ensure_ascii=False, separators=(",", ":")
    )
    return {
        **architecture,
        **vocab_interface,
        "architecture_fingerprint": _hash_text(architecture_json),
        "vocab_interface_fingerprint": _hash_text(interface_json),
        "fingerprint": _hash_text(full_json),
    }
...
class CLEvaluatorCL:
    ...
    def __init__(
        ...
        *,
        ...
        seen_tokenizer_fingerprints: set[str] = set()
        ...
        for source in tokenizer_sources:
            ...
            if not fp:
                raise RuntimeError(f"Tokenizer fingerprint missing during preflight: {source}")
            if fp in seen_tokenizer_fingerprints:
                continue
            seen_tokenizer_fingerprints.add(fp)
        ...
        if self.tokenizer_policy == "fixed" and len(self.compatibility_tokenizers) != 1:
            fingerprints = [
                meta.get("fingerprint") for meta in self.compatibility_tokenizer_metadata
            ]
            raise RuntimeError(
                "tokenizer_policy='fixed' was requested, but tokenizer preflight found "
                f"multiple distinct tokenizer fingerprints: {fingerprints}."
            )
    ...
    def _validate_model_consistency(
        ...
        *,
        ...
        architecture_fingerprint = model_metadata.get("architecture_fingerprint")
        if not isinstance(architecture_fingerprint, str) or not architecture_fingerprint:
            raise RuntimeError(
                f"Model architecture fingerprint is missing for {ckpt_path}."
            )
        ...
        if self.model_reference is None:
            ...
            if self.wandb_run is not None:
                self.wandb_run.summary[
                    "eval/model_architecture_fingerprint"
                ] = architecture_fingerprint
            return

        expected = self.model_reference.get("architecture_fingerprint")
        if architecture_fingerprint != expected:
            raise RuntimeError(
                "Model backbone architecture inconsistency across CL checkpoints. "
                f"Expected architecture fingerprint {expected}, got "
                f"{architecture_fingerprint} for {ckpt_path}. Vocabulary size and "
                "embedding rows may change with a learned tokenizer, but hidden "
                "architecture fields may not."
            )
    ...
    def _validate_tokenizer_consistency(
        ...
        *,
        ...
        fingerprint = tokenizer_metadata.get("fingerprint")
        if not isinstance(fingerprint, str) or not fingerprint:
            raise RuntimeError(f"Tokenizer fingerprint is missing for {ckpt_path}.")
        ...
        if self.tokenizer_reference is None:
            ...
            if self.wandb_run is not None:
                self.wandb_run.summary["eval/base_tokenizer_fingerprint"] = fingerprint
            return

        expected = self.tokenizer_reference.get("fingerprint")
        if self.tokenizer_policy == "fixed" and fingerprint != expected:
            raise RuntimeError(
                "Tokenizer inconsistency across CL checkpoints under "
                "tokenizer_policy='fixed'. "
                f"Expected fingerprint {expected}, got {fingerprint} for "
                f"{ckpt_path}. Use tokenizer_policy='checkpoint' only when the "
                "tokenizer is intentionally updated and saved with each checkpoint."
            )

        if self.tokenizer_policy == "checkpoint" and fingerprint != expected:
            diff = _tokenizer_metadata_diff(
                self.tokenizer_reference, tokenizer_metadata
            )
            self.accelerator.print(
                "[CL Eval] checkpoint-specific tokenizer accepted: "
                f"checkpoint={ckpt_path}, fingerprint={fingerprint}, "
                f"changed_fields={sorted(diff)}"
            )
            record["difference_from_base"] = diff
    ...
    def _validate_dataset_identity(
        ...
        *,
        ...
        """Enforce identical frozen evaluation content across checkpoints.

        ``eval_content_sha256`` hashes the ordered raw prompt/reference pairs and
        is the authoritative dataset-identity signal.  ``eval_policy_hash`` is an
        audit hash for manifest-construction compatibility; it must not by itself
        invalidate an otherwise byte-identical frozen evaluation set because fast
        tokenizer runtime padding/truncation state can change backend serialization.
        """
        signature = raw_metrics.get("eval_content_sha256")
        count_value = raw_metrics.get("eval_num_examples")
        if not isinstance(signature, str) or not signature:
            raise RuntimeError(
                f"[{domain}] EvalRunner did not return eval_content_sha256. "
                "Use the companion frozen-manifest eval_stability.py revision so the same raw intended "
                "prompt/reference set is enforced across checkpoints."
            )
        try:
        ...
        current = {
            ...
            "requested_max_length": raw_metrics.get("eval_requested_max_length"),
            "effective_max_length": raw_metrics.get("eval_effective_max_length"),
            "truncate_source": raw_metrics.get("eval_truncate_source"),
            "truncate_strategy": raw_metrics.get("eval_truncate_strategy"),
            "max_eval_samples": raw_metrics.get("eval_max_eval_samples"),
            "schema_version": raw_metrics.get("eval_schema_version"),
            "policy_hash": raw_metrics.get("eval_policy_hash"),
            "manifest_type": raw_metrics.get("eval_manifest_type"),
            "duplicate_policy": raw_metrics.get("eval_duplicate_policy"),
            "prompt_compatibility": raw_metrics.get("eval_prompt_compatibility"),
            "reference_used_for_prompt_construction": raw_metrics.get(
                "eval_reference_used_for_prompt_construction"
            ),
            "language": raw_metrics.get("eval_language"),
            "eval_family": raw_metrics.get("eval_family"),
            "task_type": raw_metrics.get("eval_task_type"),
        }
        ...
        # Hard failure remains for any real frozen-content change.
        content_keys = ("count", "content_sha256")
        content_diff = {
            key: {"base": reference.get(key), "current": current.get(key)}
            for key in content_keys
            if reference.get(key) != current.get(key)
        }
        if content_diff:
            raise RuntimeError(
                f"[{domain}] Evaluation content changed across checkpoints: "
                f"{content_diff}, checkpoint={ckpt_path}. Do not average across "
                "different frozen prompt/reference sets."
            )

        # All semantic manifest metadata other than the audit-only policy hash must
        # also remain unchanged.  This catches genuine split/preprocessing drift.
        semantic_keys = tuple(
            key for key in current.keys()
            if key not in {"count", "content_sha256", "policy_hash"}
        )
        semantic_diff = {
            key: {"base": reference.get(key), "current": current.get(key)}
        ...
        if semantic_diff:
            raise RuntimeError(
                f"[{domain}] Evaluation manifest metadata changed across checkpoints: "
                f"{semantic_diff}, checkpoint={ckpt_path}. The raw content hash is "
                "unchanged, but the evaluation policy metadata is inconsistent; "
                "inspect preprocessing before reporting results."
            )

        # A policy-hash-only difference is non-fatal when the frozen raw content
        # and all semantic policy fields are identical.  Record and surface it so
        # the run remains auditable instead of silently ignoring the discrepancy.
        if current.get("policy_hash") != reference.get("policy_hash"):
            self.accelerator.print(
                f"[CL Eval][{domain}] policy_hash changed but frozen evaluation "
                f"content is identical; continuing safely. base="
                f"{reference.get('policy_hash')}, current={current.get('policy_hash')}, "
