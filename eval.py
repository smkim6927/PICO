...
def law_preprocess_function(examples, domain, shot_type, shot_examples, tokenizer, max_length):
    """
...
def medi_preprocess_function_new(examples, domain, shot_type, shot_examples, tokenizer, max_length=256):
    """
...
def qa_preprocess_function(examples, domain, shot_type, shot_examples, tokenizer, max_length=max_length):
    """
...
class Eval:
    ...
    def evaluate(self, domain="eng_legal", shot_type="zero-shot"):
        ...
        if self.accelerator.is_main_process:
            wandb.init(project="Evaluation", name=f"{self.model_name}_{domain}_{shot_type}", config={"shots": shot_type})

        # 1) 데이터셋 로드
        if domain == "eng_medical": 
          dataset = get_dataset(domain, split="test")
        else: dataset = get_dataset(domain, split="train")
